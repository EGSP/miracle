...
1. **Временные метки переводятся на `DateTime`.** Идиоматично для Prisma/Postgres.
   Влечёт правку `DbEntity` и мест на фронте, где ожидается число (см. §4 и §8).
...
## 4. Временные метки: переход на `DateTime`

### Что меняется в типах

Тип `DbEntity` в `@miracle/types` (`types/src/db.ts`) сейчас:
...
После перехода временные метки в базе — `DateTime`. Prisma отдаёт `Date`, который при
сериализации в JSON превращается в **ISO-строку**, а не в число.

### Принятое решение: нативный формат (ISO-строки)

`DbEntity.createdAt` становится `string` (ISO-8601). Бэкенд отдаёт временные метки в том
виде, в каком их сериализует Prisma, — без конвертации на проводе. Это идиоматично для
Postgres/Prisma и архитектурно чище.

```ts
export type DbEntity = {
  id: string;
  createdAt: string;        // ISO-8601, напр. "2026-06-01T12:00:00.000Z"
  updatedAt: string;
  deletedAt?: string | null;
};
```

Цена решения — ревизия фронта в местах, где сейчас выполняется арифметика с таймстемпом
или `new Date(number)`. Эти места правятся отдельной задачей после переезда бэкенда;
поломка ожидаема и устраняется быстро. Найти их можно поиском по фронту обращений к
`createdAt`/`updatedAt`/`deletedAt` (см. §8 и список затронутых файлов в §6.1).
...
1. **Формат временных меток** (§4): нативный ISO-формат. `DbEntity.createdAt: string`,
   без конвертации на бэке. Фронт правится отдельной задачей после переезда.
