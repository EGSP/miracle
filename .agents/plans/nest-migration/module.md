...
### Группировка по смыслу (когда файлов много)

Когда в домене накапливается несколько файлов **одного концерна** (сервис + его чистые хелперы/экстракторы/мапперы), группируй их в подпапку, **названную по смыслу концерна**, а не по типу файла:

```
files-content/
  files-content.module.ts
  files-content.service.ts        # персистентность коллекции
  files-content.controller.ts
  count-tokens.ts                 # отдельный микро-концерн — в корне домена
  extraction/                     # ← концерн "извлечение контента"
    extraction.service.ts
    extract-document.ts
    extract-spreadsheet.ts
    extract-text.ts
  dto/
```

Правила группировки:
1. **Имя подпапки = концерн** (`extraction/`, `pricing/`), отвечает на вопрос *«что это»*. **Запрещены** мусорные имена-вёдра по типу файла: `lib/`, `utils/`, `helpers/`, `common/` — это тот же junk-drawer-антипаттерн, что и `src/lib/` (см. `lib.md`).
2. **Заводи подпапку только когда есть, что группировать** — 2+ файла одного концерна. Один файл живёт в корне домена.
3. **Сервис концерна кладётся в ту же подпапку** (`extraction/extraction.service.ts`) — он часть концерна, а не «обвязка над ним».
4. **Без барреля** (`index.ts`) и внутри подпапки тоже — импорты прямые.
5. Одиночные кросс-доменные чистые функции — не сюда, а в `back-nest/src/common/` (правило 5 `lib.md`), и только когда реально шарятся 2+ доменами.
