...
8. **Доменные утилиты из `back/src/lib/<...>` едут внутрь домена, а не в общий каталог.** Если старый сервис/роутер импортирует из `back/src/lib/`, перенеси этот файл в каталог твоего домена (`back-nest/src/<domain>/<name>.ts`) или сделай приватным методом сервиса. Не создавай `back-nest/src/lib/` или `back-nest/src/common/` без необходимости. Полные правила, классификация и карта текущего `lib/` — в `lib.md`.
