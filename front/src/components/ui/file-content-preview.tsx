import { type FileWithMeta, isImageExtension } from "@miracle/types"
...
function usePreviewContext() {
  ...
  if (!context) {
    throw new Error(
      "FileContentPreview compound components must be used within FileContentPreview root.",
    )
  }
  return context
}
...
function ImagePreview({ className }: PreviewSlotProps) {
  ...
  if (!isAvailable || kind !== "image") {
    return (
      <PreviewFallback className={className} message="Изображение недоступно для предпросмотра" />
    )
  }
  ...
  return (
    <div className={cn("relative min-h-52 border border-border bg-muted/20", className)}>
      ...
      <iframe
        ...
        className={cn(
          "h-[60vh] w-full transition-opacity",
          isLoading ? "opacity-0" : "opacity-100",
        )}
      />
    </div>
...
function FileContentPreviewRoot({
  file,
  resolveUrl,
  children,
  className,
  ...
  return (
    <PreviewContext.Provider value={contextValue}>
      <div className={className}>{children ?? <AutoRenderer />}</div>
