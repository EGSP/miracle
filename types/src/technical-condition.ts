export type TechnicalCondition = {
    ...
    slotRules?: SlotRule[];
    /**
     * Примеры расшифровки фрагментов из заявок: как текст заказчика сопоставляется
     * с параметрами условного обозначения. Дополняет `slotRules` (норма из ТУ)
     * практическими подсказками — типичные формулировки, готовые шифры в названии,
     * случаи, когда похожий текст не является требованием. Передаётся в промпт
     * `analyse-designation` перед правилами слотов.
     */
    designationDecodeExamples?: string;
