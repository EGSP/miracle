...
export type OrderPositionData = {
    /**
     * Требования к позиции — массив дословно захваченных строк (по одной на пункт/строку
     * требования; для опросных листов — по строке "Параметр: Значение"). Пустой массив — требований
     * нет. Массив (а не одна строка) намеренно: подталкивает извлечение перечислять все пункты, а не
     * сжимать длинное ТЗ в абзац. Интерпретация — на последующих шагах (designation/ТУ).
     */
    requirements: string[];
