...
const PositionZodSchema = z.object({
    ...
    requirements: z
        .array(z.string())
        .describe(
            'Все требования к позиции дословно, по одному элементу на пункт/строку (для опросных листов — строки "Параметр: Значение"); [] если требований нет',
        ),
...
Правила захвата требований (requirements — МАССИВ строк):
- Захвати ВСЕ требования к этой позиции ДОСЛОВНО, по одному элементу массива на пункт/строку требования. Ничего не суммируй, не сокращай и не выбрасывай пункты.
- Опросные листы и таблицы вида «Параметр | Значение»: КАЖДУЮ строку сохраняй отдельным элементом как "Параметр: Значение" (например, "Срок службы изделия: 30 лет"). Это сохранение источника, а не интерпретация. Не сворачивай таблицу в одну фразу.
- Условные обозначения/шифр НЕ разворачивай и не дроби на отдельные параметры — но строки-параметры опросного листа захватывай полностью, как написано.
- Блоки с отметками выбора («Выбрано:»/«Не выбрано:», чекбоксы): в требования включай ТОЛЬКО выбранный вариант (отдельным элементом, дословно вместе с его заголовком/номером пункта); невыбранные альтернативы отбрасывай.
- Если требований к позиции нет — пустой массив [].

Правила захвата прочего:
        ...
        const llm = defineJob(
            ...
            (input: ExtractInput): Effect.Effect<OrderPosition[], unknown, JobEnv> =>
                Effect.gen(function* () {
                    ...
                    // submit-once: opId сохраняется в memo ДО ожидания, поэтому при рестарте job
                    // продолжает polling существующей операции, а не отправляет LLM повторно.
                    ...
                    const opId = yield* Option.match(savedOpId, {
                        ...
                        onNone: () =>
                            Effect.gen(function* () {
                                yield* memo.set('finalPrompt', { system, user: userText });
